...
export async function fetchRelatedData(supabase, sales, filters) {
  console.log("🔗 Fetching additional related data...");
  if (!sales || sales.length === 0) {
    console.log("🔗 No sales data to process");
    return sales;
  }
  console.log(`🔗 Processing ${sales.length} sales records for additional data`);
  ...
  if (filters.includeCreator && !sales[0]?.profiles) {
    fetchPromises.push(fetchCreators(supabase, sales));
  }
  if ((filters.includeImages || filters.includeStoveImage || filters.includeAgreementImage) && !sales[0]?.stove_image && !sales[0]?.agreement_image) {
    fetchPromises.push(fetchImages(supabase, sales));
  }
  // Execute all fetches in parallel
  if (fetchPromises.length > 0) {
    console.log(`🚀 Executing ${fetchPromises.length} additional fetch operations in parallel...`);
...
async function fetchOrganizations(supabase, sales) {
  console.log("🏢 Fetching organizations optimized...");
  const orgIds = [
    ...new Set(sales.map((sale)=>sale.organization_id).filter((id)=>id !== null && id !== undefined))
  ];
  if (orgIds.length === 0) {
  ...
  const { data: organizations, error: orgsError } = await supabase.from("organizations").select("id, name, created_at, partner_email").in("id", orgIds);
  if (orgsError) {
  ...
  const orgMap = new Map(organizations?.map((org)=>[
      org.id,
      org
    ]) || []);
...
async function fetchAddresses(supabase, sales) {
  console.log("📍 Fetching addresses optimized...");
  const addressIds = [
    ...new Set(sales.map((sale)=>sale.address_id).filter((id)=>id !== null && id !== undefined))
  ];
  ...
  const { data: addresses, error: addressError } = await supabase.from("addresses").select("id, city, state, street, country, latitude, longitude, full_address, created_at").in("id", addressIds);
  if (addressError) {
  ...
  const addressMap = new Map(addresses?.map((addr)=>[
      addr.id,
      addr
    ]) || []);
...
async function fetchCreators(supabase, sales) {
  console.log("👤 Fetching creators optimized...");
  const creatorIds = [
    ...new Set(sales.map((sale)=>sale.created_by).filter((id)=>id !== null && id !== undefined))
  ];
  ...
  const { data: creators, error: creatorsError } = await supabase.from("profiles").select("id, full_name, email, phone, role").in("id", creatorIds);
  if (creatorsError) {
...
async function fetchImages(supabase, sales) {
  console.log("🖼️ Fetching images optimized...");
  // Collect all unique image IDs
  const stoveImageIds = sales.map((sale)=>sale.stove_image_id).filter((id)=>id !== null && id !== undefined);
  const agreementImageIds = sales.map((sale)=>sale.agreement_image_id).filter((id)=>id !== null && id !== undefined);
  const allImageIds = [
    ...new Set([
      ...stoveImageIds,
      ...agreementImageIds
  ...
  const { data: images, error: imagesError } = await supabase.from("uploads").select("id, public_id, url, type, created_by, created_at").in("id", allImageIds);
  if (imagesError) {
...
function logRelatedDataResults(sales) {
