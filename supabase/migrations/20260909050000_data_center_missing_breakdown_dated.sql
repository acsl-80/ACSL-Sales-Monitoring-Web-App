-- ===========================================================================
-- Data Center, slice F3b: the Complete card's breakdown knows the dated rules.
--
-- sales.incomplete_by_missing had one bucket per configured required field
-- and one for the evidence. Since slice F3a the completeness rule also reads
-- the dated rows of public.sale_field_rules, so a sale incomplete only for a
-- dated field counted in sales.incomplete and had no bucket. This re-creates
-- compute_metrics from 20260908030000 with one bucket per dated rule, keyed
-- by the dictionary key the Missing facet and missing_predicate accept.
