-- ===========================================================================
-- Phase 24, slice 6a: the control centre's numbers, in the one engine.
--
-- 1. compute_metrics gains the pool family (callable, partners, recall-due,
--    recently digitised, both by partner, never called) and a second argument:
--    a run asked for `array['pool']` computes that family and stops, which is
--    what the board's Recompute presses. The full run is unchanged. The
--    function is dropped and recreated because the signature changed; the
--    one caller passes one argument and still resolves.
-- 2. v_agent_activity: per agent, the latest draft save (and which record),
--    the latest batch activity, the latest attempt, and today's attempts.
--    Presence is derived from these at read time against two settings; no
--    channel, no heartbeat beyond the editor's own autosave.
-- 3. Settings: the recent window, the two presence windows, the refresh
--    interval the page polls at (0 turns polling off).
-- ===========================================================================

insert into data_center.workflow_config (key, value, description) values
  ('pool.recent_days', '7'::jsonb,
   'A record digitised within this many days counts as recently added on the board and the pool by partner.'),
  ('presence.working_within_minutes', '10'::jsonb,
   'An agent who saved a call draft or logged a call within this many minutes reads as Working.'),
...
comment on view data_center.v_agent_activity is
  'Per call agent: the latest draft save and its record, the latest batch activity, the latest attempt, attempts today, and the newest of those as last_seen_at. Presence is derived from last_seen_at against presence.* settings at read time.';

-- ---------------------------------------------------------------------------
-- The metric engine, with the pool family and a partial-run argument.
-- ---------------------------------------------------------------------------

drop function if exists data_center.compute_metrics(uuid);
