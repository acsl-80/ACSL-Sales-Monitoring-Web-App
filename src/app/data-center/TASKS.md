...
- [ ] F3a The rules (A4, A5, D31): public.sale_field_rules, one row per field with the day it becomes mandatory and which rules read it; calculate_sale_status and completeness_predicate read it; the trigger owns the status (create-sale, update-sale and the backfill read it back); the dictionary endpoint serves the table's dates; the shared validator refuses at the forms; a Field rules panel in Settings. Host lane on his word. PR #83, green in batches (6 plus 17 plus 29), three-finder review folded in, awaiting the merge word. Go-live date seeded as 2026-09-08, movable in Settings.
