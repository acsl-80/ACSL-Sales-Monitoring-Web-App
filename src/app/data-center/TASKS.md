...
- [ ] F4 The external API's second shape with the Stove DB names: one shared shape from the dictionary's stoveDbName, served as stove_db by get-sales-advanced and end-user-records-api; format1 takes the name from its two columns and fills cpa; the docs page shows the third shape; CSV headers already from the dictionary (F1). The old shapes stay until the analysts move (Q3). PR #85, green in batches (4 plus 30 plus 4), two-finder review folded in, awaiting the merge word.
