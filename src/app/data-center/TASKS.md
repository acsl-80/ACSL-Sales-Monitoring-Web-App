...
- [ ] F2b The transfer's order model (D19): additive columns on stove_transfer_history written by both ERP syncs through one shared matcher, exposed on v_transfers, preselected on the bench with its provenance line, prefilled on the sheet. Host lane on his word. PR #82, green in batches, review folded in, awaiting the merge word. The rail's model chip waits for F3, which reshapes the bench options.
