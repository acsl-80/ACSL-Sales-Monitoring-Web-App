...
- [ ] F5 The phone app guide and the payload contract: `docs/contractor/sales-mobile-field-alignment.md` is the canonical guide, its field table generated from the dictionary (a copy sits beside the repos); `e2e/host-payload-contract.spec.ts` reads the keys the two host functions accept from their source, sends every key through create-sale and update-sale and reads it back, and scans the phone app's sources for every key the rules require today. The dictionary's seed dates now match the rules table. PR #86, green on the sandbox (4 plus 13 plus 10), two-finder review folded in, awaiting the merge word. The scan reports endUserFirstName and salesAgentName missing from the phone app, required from 2026-09-08.
